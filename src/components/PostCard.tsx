  ...
  return (
    <article className="post-card">
      <div className="post-header">
        <div className="post-avatar">{post.creator.slice(2, 4).toUpperCase()}</div>
      ...
      <div className="post-body">
        ...
        <div className="post-actions">
          ...
          >
            <span className="like-icon">{isLiking ? "⏳" : isLiked ? "♥" : "♡"}</span>
            <span>{post.likes.toString()}</span>
          </button>
          <span className="post-earned">✦ {earned} ETH</span>
        </div>
      </div>
    </article>
  );
